    ...
    return (
        <div className="container TrendingProducts">
            ...
            <div className="TrendingProductsHeader d-flex justify-content-between align-items-center mb-4">
                <h4>Baby care collection</h4>
                {/* <a href="#" className="">See all products <span><FaArrowRight /></span></a> */}
                <Link to="/product">See all products <span><FaArrowRight /></span></Link>
            </div>
            ...
            <div className="TrendingProductsList">
                <Tab.Container activeKey={activeTab} onSelect={(k) => setActiveTab(k)}>
                    ...
                    <Nav variant="pills" className="mb-5 TrendingProductsTab flex-wrap">
                        {categories.map((cat) => (
                            <Nav.Item key={cat}>
                                <Nav.Link className="TrendingProductsCategory" eventKey={cat}>{cat}</Nav.Link>
                            </Nav.Item>
                        ))}
                    </Nav>
                    ...
                    <Tab.Content className="TrendingProductsList">
                        {categories.map((cat) => (
                            <Tab.Pane key={cat} eventKey={cat}>
                                <Row>
                                    {products.filter((item) => item.category === cat).splice(0, 8).map((item) => (
                                        <Col key={item.id} xs={12} sm={6} md={4} lg={3} className="mb-4 px-2" style={{ marginTop: "60px" }}>
                                            <Card className="TrendingProductsCard" >
                                                <div className="TrendingProductsCardImg"><Card.Img variant="top" src="/Img/bbycare.png" /></div>
                                                <Card.Body className="TrendingProductsCardContent">
                                                    <div className="CardContentHeading  d-flex">
                                                        <h2>{item.category}</h2>
                                                        <div className="d-flex align-items-center"><IoMdStar style={{ color: "#F6E200", fontSize: "18px", marginRight: "5px" }} /> <p className="mb-0">({item.rating})</p></div>
                                                    </div>
                                                    <Card.Title className="CardContentTitle mb-0">{item.title}</Card.Title>
                                                </Card.Body>
                                            </Card>
                                        </Col>
                                    ))}
